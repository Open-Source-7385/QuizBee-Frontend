// quizbee-frontend/src/app/quizzies/presentation/views/quizz-create/quizz-create.ts
...
import { AuthService } from '../../../../shared/core/auth.service'; // ✅ AGREGAR
import { UserStatsService } from '../../../../shared/core/user-stats.service'; // ✅ AGREGAR
...
export class QuizzCreate implements OnInit {
  ...
  constructor(
    ...
    private authService: AuthService, // ✅ CAMBIAR NOMBRE
  ...
  async saveQuiz(): Promise<void> {
    ...
    if (!currentUser) {
      this.errorMessage.set('Debes iniciar sesión para crear un quiz');
    ...
    this.quizService.createQuiz(quizPayload as Quiz).subscribe({
      next: (created) => {
        this.saving.set(false);
        this.successMessage.set(`✅ Quiz "${created.title}" creado por ${currentUser.name}`);
        localStorage.removeItem('quiz_draft');

        // ✅ Agregar puntos al usuario por crear quiz
        this.userStatsService.addPoints(50);
        console.log('✅ +50 puntos por crear quiz');
        setTimeout(() => this.router.navigate(['/quizz']), 1500);
      },
      error: (err) => {
        console.error(err);
        this.saving.set(false);
